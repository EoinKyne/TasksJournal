...
class JournalEntryAPITest(APITestCase):
    ...
    def test_create_entry(self):
        ...
        self.assertEqual(JournalEntry.objects.first().title, 'Test Case entry')

    def test_get_entries(self):
        JournalEntry.objects.create(title='Test', description='Description')
        url = reverse('journalentry-list')
    ...
    def test_get_entry_by_id(self):
        entry = JournalEntry.objects.create(title='Test', description='Description')

        response = self.client.get(reverse('journalentry-detail', args=[entry.id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_entry(self):
        ...
        response = self.client.delete(reverse('journalentry-detail', args=[entry.id]))
